...
// 工具函数：安全的聊天列表合并，避免重复
const safeMergeChats = (
  existingChats: ChatInfoDTO[],
  newChats: ChatInfoDTO[]
): ChatInfoDTO[] => {
...
export const isMessageFromMe = (
  message: MessageDTO | ClientMessageDTO
): boolean => {
...
export const useChatStore = create<ChatStore>((set, get) => ({
        ...
        chatMessagesMap: {
          ...
          [chatId]: {
            ...
            oldestMessageId:
              messages.length > 0
                ? messages[0].id
                : currentChatState.oldestMessageId,
          },
        },
      });
    } catch {
  ...
  loadChatList: async (lastMessageId?: string) => {
    ...
    try {
      ...
      const result = await chatService.getChatList(lastMessageId, 10);
  ...
  ensureFriendChatAndNavigate: async (friendUserId: string) => {
    ...
    // 检查缓存的聊天列表中是否包含相应聊天
    const existingChatIndex = currentState.chats.findIndex(
      (chat) => chat.id === chatInfo.id
    );
    ...
    if (existingChatIndex >= 0) {
      ...
      const otherChats = currentState.chats.filter(
        (chat) => chat.id !== chatInfo.id
      );
